...
exports.initNewRolesAndPermissions = initNewRolesAndPermissions;
...
async function initNewRolesAndPermissions() {
    try {
        logger_1.logger.info('开始初始化新的角色和权限配置...');
        // 创建权限
        ...
        for (const roleData of defaultRoles) {
            ...
            if (!role) {
                ...
                logger_1.logger.info(`更新角色: ${role.name} (${role.code})`);
            }
            // 分配权限
        ...
        logger_1.logger.info('✅ 新的角色和权限配置初始化完成');
    }
    catch (error) {
        logger_1.logger.error('❌ 角色和权限配置初始化失败:', error);
        throw error;
    }
}
//# sourceMappingURL=newRolesAndPermissions.js.map