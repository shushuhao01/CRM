...
exports.initRolesAndPermissions = initRolesAndPermissions;
...
async function createPermissions(permissions, parent = null) {
    if (!database_1.AppDataSource) {
        throw new Error('AppDataSource is not initialized');
    }
    const permissionRepository = database_1.AppDataSource.getTreeRepository(Permission_1.Permission);
    ...
    for (const permData of permissions) {
        ...
        if (!permission) {
            ...
            logger_1.logger.info(`创建权限: ${permission.name} (${permission.code})`);
        }
        if (permission) {
...
async function initRolesAndPermissions() {
    try {
        ...
        if (process.env.NODE_ENV === 'production') {
            logger_1.logger.info('生产环境跳过角色权限初始化');
            return;
        }
        logger_1.logger.info('开始初始化角色和权限...');
        if (!database_1.AppDataSource) {
            throw new Error('AppDataSource is not initialized');
        }
        // 创建权限
        logger_1.logger.info('创建默认权限...');
        const allPermissions = await createPermissions(defaultPermissions);
        logger_1.logger.info(`成功创建 ${allPermissions.length} 个权限`);
        // 创建角色
        logger_1.logger.info('创建默认角色...');
        ...
        for (const roleData of defaultRoles) {
            ...
            if (!role) {
                ...
                if (savedRole) {
                    role = savedRole;
                    logger_1.logger.info(`创建角色: ${role.name} (${role.code})`);
                }
            }
            // 分配权限
            ...
            logger_1.logger.info(`为角色 ${role.name} 分配了 ${role.permissions.length} 个权限`);
        }
        logger_1.logger.info('✅ 角色和权限初始化完成');
    }
    catch (error) {
        logger_1.logger.error('❌ 角色和权限初始化失败:', error);
        throw error;
    }
}
//# sourceMappingURL=initRolesAndPermissions.js.map