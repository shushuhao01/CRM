...
class LogisticsController {
    constructor() {
        this.expressAPIService = ExpressAPIService_1.ExpressAPIService.getInstance();
    }
    get logisticsTrackingRepository() {
        return database_1.AppDataSource.getRepository(LogisticsTracking_1.LogisticsTracking);
    }
    get logisticsTraceRepository() {
        return database_1.AppDataSource.getRepository(LogisticsTrace_1.LogisticsTrace);
    }
    get orderRepository() {
        return database_1.AppDataSource.getRepository(Order_1.Order);
    }
    // 获取物流列表
    async getLogisticsList(req, res) {
        try {
            const { page = 1, pageSize = 20, trackingNo, companyCode, status, startDate, endDate, orderId } = req.query;
    ...
    // 创建物流跟踪
    async createLogisticsTracking(req, res) {
        try {
            const { orderId, trackingNo, companyCode, companyName } = req.body;
            // 检查订单是否存在
            ...
            const logisticsTracking = this.logisticsTrackingRepository.create({
                ...
                companyCode,
                companyName,
                status: LogisticsTracking_1.LogisticsStatus.PENDING,
    ...
    // 查询物流轨迹
    async getLogisticsTrace(req, res) {
        try {
            ...
            if (!tracking) {
                // 如果没有记录，尝试直接查询API
                const apiResult = await this.expressAPIService.queryExpress(trackingNo, companyCode || 'auto');
                return res.json({
                    code: 200,
                    message: '查询成功',
                    data: apiResult
    ...
    // 批量同步物流状态
    async batchSyncLogistics(req, res) {
        try {
            ...
            const results = await Promise.allSettled(trackingNumbers.map(async (trackingNo) => {
                const tracking = await this.logisticsTrackingRepository.findOne({
                    where: { trackingNo: trackingNo }
                });
                if (tracking) {
    ...
    // 查询物流信息（内部方法）
    async queryLogisticsInfo(trackingId) {
        try {
            const tracking = await this.logisticsTrackingRepository.findOne({
                where: { id: trackingId }
            });
    ...
    // 获取支持的快递公司列表
    async getSupportedCompanies(req, res) {
        try {
    ...
    // 映射API状态到内部状态
    mapApiStatusToLogisticsStatus(apiStatus) {
        const statusMap = {
            'pending': LogisticsTracking_1.LogisticsStatus.PENDING,
            'picked_up': LogisticsTracking_1.LogisticsStatus.PICKED_UP,
            'in_transit': LogisticsTracking_1.LogisticsStatus.IN_TRANSIT,
            'out_for_delivery': LogisticsTracking_1.LogisticsStatus.OUT_FOR_DELIVERY,
            'delivered': LogisticsTracking_1.LogisticsStatus.DELIVERED,
            'exception': LogisticsTracking_1.LogisticsStatus.EXCEPTION,
            'rejected': LogisticsTracking_1.LogisticsStatus.REJECTED,
            'returned': LogisticsTracking_1.LogisticsStatus.RETURNED
        };
        return statusMap[apiStatus] || LogisticsTracking_1.LogisticsStatus.PENDING;
    }
}
