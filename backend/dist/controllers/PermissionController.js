...
class PermissionController {
    get permissionRepository() {
        return database_1.AppDataSource.getTreeRepository(Permission_1.Permission);
    }
    // 获取权限树
    ...
    async createPermission(req, res) {
        try {
            const { name, code, description, module, type = 'menu', path, icon, sort = 0, status = 'active', parentId } = req.body;
            // 检查权限编码是否已存在
