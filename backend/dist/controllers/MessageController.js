...
// 内存存储订阅规则数据（模拟数据库）
const subscriptionRulesStorage = [
    {
        id: 1,
        departmentId: '1',
        departmentName: '销售部',
...
class MessageController {
        ...
        try {
            ...
            if (!dataSource) {
                ...
                const mockData = [
                    {
                        id: 1,
                        messageType: MessageSubscription_1.MessageType.ORDER_CREATED,
                        name: '订单创建通知',
                        description: '当有新订单创建时发送通知',
                        category: '订单管理',
                        isGlobalEnabled: true,
                        globalNotificationMethods: [MessageSubscription_1.NotificationMethod.DINGTALK, MessageSubscription_1.NotificationMethod.EMAIL],
                        departmentConfigs: [
                            {
                                id: 1,
                                departmentId: 1,
                                departmentName: '销售部',
                                isEnabled: true,
                                notificationMethods: [MessageSubscription_1.NotificationMethod.DINGTALK]
                            },
                            {
                                id: 2,
                                departmentId: 2,
                                departmentName: '客服部',
                                isEnabled: false,
                                notificationMethods: [MessageSubscription_1.NotificationMethod.EMAIL]
                            }
                        ]
                    },
                    {
                        id: 2,
                        messageType: MessageSubscription_1.MessageType.CUSTOMER_CREATED,
                        name: '客户创建通知',
                        description: '当有新客户注册时发送通知',
                        category: '客户管理',
                        isGlobalEnabled: true,
                        globalNotificationMethods: [MessageSubscription_1.NotificationMethod.WECHAT_WORK],
                        departmentConfigs: [
                            {
                                id: 3,
                                departmentId: 1,
                                departmentName: '销售部',
                                isEnabled: true,
                                notificationMethods: [MessageSubscription_1.NotificationMethod.WECHAT_WORK]
                            }
                        ]
                    }
                ];
            ...
            const subscriptionRepo = dataSource.getRepository(MessageSubscription_1.MessageSubscription);
            const departmentConfigRepo = dataSource.getRepository(MessageSubscription_1.DepartmentSubscriptionConfig);
            ...
            const result = subscriptions.map((subscription) => ({
                ...
                departmentConfigs: departmentConfigs
                    .filter((config) => config.messageType === subscription.messageType)
                    .map((config) => ({
                    id: config.id,
    ...
    // 获取部门级别的订阅配置
    async getDepartmentSubscriptions(req, res) {
        try {
    ...
    // 更新部门级别的订阅配置
    async updateDepartmentSubscription(req, res) {
        try {
            ...
            const departmentConfigRepo = dataSource.getRepository(MessageSubscription_1.DepartmentSubscriptionConfig);
            const departmentRepo = dataSource.getRepository(Department_1.Department);
            // 检查部门是否存在
            ...
            let config = await departmentConfigRepo.findOne({
                where: { messageType: messageType, department: { id: departmentId } },
                relations: ['department']
            });
            if (config) {
                // 更新现有配置
    ...
    // 批量更新部门订阅配置
    async batchUpdateDepartmentSubscriptions(req, res) {
        try {
            const dataSource = (0, database_1.getDataSource)();
            if (!dataSource) {
                res.status(500).json({
            ...
            const departmentRepo = dataSource.getRepository(Department_1.Department);
            // 删除现有配置
            await departmentConfigRepo.delete({ messageType: messageType });
            // 创建新配置
            const newConfigs = [];
    ...
    // 初始化默认消息订阅配置
    async initializeDefaultSubscriptions(req, res) {
        try {
            const dataSource = (0, database_1.getDataSource)();
            if (!dataSource) {
                res.status(500).json({ error: '数据库连接未初始化' });
            ...
            const defaultSubscriptions = [
                {
                    ...
                    category: '订单管理',
                    isGlobalEnabled: true,
                    globalNotificationMethods: [MessageSubscription_1.NotificationMethod.EMAIL, MessageSubscription_1.NotificationMethod.SYSTEM_MESSAGE]
                },
                {
                    messageType: MessageSubscription_1.MessageType.CUSTOMER_CREATED,
                    name: '客户创建',
                ...
                {
                    messageType: MessageSubscription_1.MessageType.SYSTEM_MAINTENANCE,
                    name: '系统维护',
                    description: '系统维护通知',
                    category: '系统管理',
                    isGlobalEnabled: true,
                    globalNotificationMethods: [MessageSubscription_1.NotificationMethod.EMAIL, MessageSubscription_1.NotificationMethod.ANNOUNCEMENT, MessageSubscription_1.NotificationMethod.SYSTEM_MESSAGE]
                }
            ];
    ...
    async updateAnnouncement(req, res) {
        try {
            ...
            if (targetDepartments !== undefined)
                announcement.targetDepartments = targetDepartments;
            if (startTime !== undefined)
                announcement.startTime = startTime ? new Date(startTime) : undefined;
            if (endTime !== undefined)
                announcement.endTime = endTime ? new Date(endTime) : undefined;
            if (isPinned !== undefined)
                announcement.isPinned = isPinned ? 1 : 0;
            await announcementRepo.save(announcement);
            res.json({
                success: true,
                message: '公告更新成功',
    ...
    // 订阅规则管理
    async getSubscriptionRules(req, res) {
        try {
            // 获取查询参数
            ...
            if (messageType) {
                filteredRules = filteredRules.filter(rule => rule.messageTypes.includes(messageType));
            }
    ...
    async createSubscriptionRule(req, res) {
        try {
            const { departmentId, messageTypes, notificationMethods, priority, scheduleEnabled, scheduleStart, scheduleEnd, excludeWeekends, remark } = req.body;
            // 验证必填字段
    ...
    async updateSubscriptionRule(req, res) {
        try {
            const { id } = req.params;
    ...
    async deleteSubscriptionRule(req, res) {
        try {
            const { id } = req.params;
            const ruleId = parseInt(id);
            // 查找要删除的规则
    ...
    async toggleSubscriptionRule(req, res) {
        try {
            const { id } = req.params;
    ...
    // 获取部门和成员数据
    async getDepartmentsAndMembers(req, res) {
        try {
            const dataSource = (0, database_1.getDataSource)();
            if (!dataSource) {
                // 测试模式：返回模拟部门和成员数据
     ...
     * 批量发送系统消息
     */
    async sendBatchSystemMessages(req, res) {
        try {
            const dataSource = (0, database_1.getDataSource)();
            if (!dataSource) {
                res.status(500).json({ success: false, message: '数据库未连接' });
    ...
    async getMessageStats(req, res) {
        try {
            ...
            try {
                const channelRepo = dataSource.getRepository(NotificationChannel_1.NotificationChannel);
                notificationChannelCount = await channelRepo.count();
                const logRepo = dataSource.getRepository(NotificationChannel_1.NotificationLog);
            ...
            try {
                const { PerformanceReportConfig } = await Promise.resolve().then(() => __importStar(require('../entities/PerformanceReportConfig')));
                ...
                // 今日发送的
                todayPerformanceSent = successConfigs.filter(c => c.lastSentAt && new Date(c.lastSentAt) >= today).length;
            }
            catch (_e) {
     ...
     * 获取通知渠道配置列表
     */
    async getNotificationChannels(_req, res) {
        try {
            const dataSource = (0, database_1.getDataSource)();
            if (!dataSource) {
    ...
    async createNotificationChannel(req, res) {
        try {
            ...
            const { name, channelType, config, messageTypes, targetType, targetDepartments, targetUsers, targetRoles, priorityFilter } = req.body;
            if (!name || !channelType) {
    ...
    async updateNotificationChannel(req, res) {
        try {
            ...
            const { name, isEnabled, config, messageTypes, targetType, targetDepartments, targetUsers, targetRoles, priorityFilter } = req.body;
            if (name !== undefined)
                channel.name = name;
            if (isEnabled !== undefined)
                channel.isEnabled = isEnabled ? 1 : 0;
            if (config !== undefined)
                channel.config = config;
            if (messageTypes !== undefined)
                channel.messageTypes = messageTypes;
            if (targetType !== undefined)
                channel.targetType = targetType;
            if (targetDepartments !== undefined)
                channel.targetDepartments = targetDepartments;
            if (targetUsers !== undefined)
                channel.targetUsers = targetUsers;
            if (targetRoles !== undefined)
                channel.targetRoles = targetRoles;
            if (priorityFilter !== undefined)
                channel.priorityFilter = priorityFilter;
            await channelRepo.save(channel);
            res.json({
                success: true,
                message: '通知配置更新成功',
     ...
     * 发送钉钉消息
     */
    async sendDingtalkMessage(config, message) {
        try {
     ...
     * 发送企业微信消息
     */
    async sendWechatWorkMessage(config, message) {
        try {
     ...
     * 发送邮件 - 使用nodemailer
     */
    async sendEmailMessage(config, message, subject) {
        try {
     ...
     * 发送短信 - 阿里云短信服务
     */
    async sendSmsMessage(config, _message) {
        try {
     ...
     * 获取可用的消息类型和渠道类型
     */
    async getNotificationOptions(_req, res) {
        try {
            const messageTypes = [
                // ========== 主要消息类型（始终显示）==========
                // 订单审核相关
