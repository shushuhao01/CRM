...
class DepartmentController {
    get departmentRepository() {
        ...
        return database_1.AppDataSource.getRepository(Department_1.Department);
    }
    get userRepository() {
        if (!database_1.AppDataSource) {
            throw new Error('Database not initialized');
     ...
     * 添加部门成员
     */
    async addDepartmentMember(req, res) {
        try {
     ...
     * 移除部门成员
     */
    async removeDepartmentMember(req, res) {
        try {
    ...
    async getDepartmentStats(req, res) {
        try {
            ...
            const totalMembers = await this.userRepository.count({
                where: { departmentId: (0, typeorm_1.Not)((0, typeorm_1.IsNull)()) }
            });
            const stats = {
                totalDepartments,
                activeDepartments,
                ...
                departmentsByType: {
                    ...
                    '子部门': await this.departmentRepository.count({
                        where: { parentId: (0, typeorm_1.Not)((0, typeorm_1.IsNull)()) }
                    })
                }
            };
            res.json({
     ...
     * 更新部门权限
     */
    async updateDepartmentPermissions(req, res) {
        try {
            const { id } = req.params;
