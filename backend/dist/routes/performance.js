...
router.get('/shares', async (req, res) => {
    try {
        ...
        const shares = await database_1.AppDataSource.query(sql, params);
        // 获取总数
...
router.post('/shares', async (req, res) => {
    try {
        ...
        // 验证分成比例总和
        const totalPercentage = shareMembers.reduce((sum, m) => sum + m.percentage, 0);
        ...
        // 插入分享记录
        await database_1.AppDataSource.query(`INSERT INTO performance_shares
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`, [shareId, shareNumber, orderId, orderNumber, orderAmount, totalShareAmount,
        ...
        for (const member of shareMembers) {
            ...
            await database_1.AppDataSource.query(`INSERT INTO performance_share_members
 ...
 * @desc 确认业绩分享
 */
router.post('/shares/:id/confirm', async (req, res) => {
    try {
        const { id } = req.params;
        const currentUser = req.user;
        // 更新成员状态
        await database_1.AppDataSource.query(`UPDATE performance_share_members SET status = 'confirmed', confirm_time = NOW()
       WHERE share_id = ? AND user_id = ?`, [id, currentUser?.userId]);
        // 检查是否所有成员都已确认
        const [pendingCount] = await database_1.AppDataSource.query(`SELECT COUNT(*) as count FROM performance_share_members WHERE share_id = ? AND status != 'confirmed'`, [id]);
...
router.get('/stats', async (req, res) => {
    try {
        const currentUser = req.user;
        const [totalResult] = await database_1.AppDataSource.query(`SELECT COUNT(*) as total, SUM(order_amount) as totalAmount FROM performance_shares`);
        const [pendingResult] = await database_1.AppDataSource.query(`SELECT COUNT(*) as count FROM performance_shares WHERE status = 'active'`);
        const [completedResult] = await database_1.AppDataSource.query(`SELECT COUNT(*) as count FROM performance_shares WHERE status = 'completed'`);
        // 用户相关统计
        const [userResult] = await database_1.AppDataSource.query(`SELECT COUNT(DISTINCT ps.id) as count, SUM(psm.share_amount) as amount
...
router.get('/analysis', async (req, res) => {
    try {
        // 获取最近7天趋势
        const trendData = await database_1.AppDataSource.query(`SELECT DATE(created_at) as date,
              COUNT(*) as orders,
       ...
       ORDER BY date`);
        // 订单状态分布
        const statusDistribution = await database_1.AppDataSource.query(`SELECT status, COUNT(*) as count FROM orders GROUP BY status`);
        // 汇总数据
        const [summary] = await database_1.AppDataSource.query(`SELECT COUNT(*) as totalOrders,
              SUM(total_amount) as totalAmount,
 ...
 * @desc 获取个人业绩分析数据
 */
router.get('/analysis/personal', async (req, res) => {
    try {
        const currentUser = req.user;
        const userId = req.query.userId || currentUser?.userId;
 ...
 * @desc 获取公司业绩分析数据
 */
router.get('/analysis/company', async (_req, res) => {
    try {
        const [stats] = await database_1.AppDataSource.query(`SELECT
         COUNT(*) as orderCount,
         SUM(total_amount) as orderAmount,
