...
router.get('/metrics', async (req, res) => {
    try {
       ...
       WHERE o.created_at >= ? AND o.created_at <= ?${userCondition}`, [todayStart, todayEnd, ...params]);
        // 🔥 昨日订单数据（用于计算环比）
        const yesterdayOrdersData = await database_1.AppDataSource.query(`SELECT total_amount as totalAmount, status, mark_type as markType
       FROM orders o
       WHERE o.created_at >= ? AND o.created_at <= ?${userCondition}`, [yesterdayStart, yesterdayEnd, ...params]);
        // 本月订单数据
       ...
       WHERE o.created_at >= ? AND o.created_at <= ?${userCondition}`, [monthStart, todayEnd, ...params]);
        // 🔥 上月订单数据（用于计算环比）
        const lastMonthOrdersData = await database_1.AppDataSource.query(`SELECT total_amount as totalAmount, status, mark_type as markType
       FROM orders o
       WHERE o.created_at >= ? AND o.created_at <= ?${userCondition}`, [lastMonthStart, lastMonthEnd, ...params]);
        // 过滤有效订单（计入下单业绩）
        ...
        const validYesterdayOrders = yesterdayOrdersData.filter((o) => isValidForOrderPerformance(o));
        const yesterdayOrders = validYesterdayOrders.length;
        const yesterdayRevenue = validYesterdayOrders.reduce((sum, order) => sum + (Number(order.totalAmount) || 0), 0);
        ...
        const yesterdayShippedOrders = yesterdayOrdersData.filter((o) => isValidForShipmentPerformance(o));
        const yesterdayDeliveredOrders = yesterdayOrdersData.filter((o) => isValidForDeliveryPerformance(o));
        const lastMonthShippedOrders = lastMonthOrdersData.filter((o) => isValidForShipmentPerformance(o));
        const lastMonthDeliveredOrders = lastMonthOrdersData.filter((o) => isValidForDeliveryPerformance(o));
        const monthlyDeliveredCount = monthlyDeliveredOrders.length;
        const monthlyDeliveredAmount = monthlyDeliveredOrders.reduce((sum, o) => sum + (Number(o.totalAmount) || 0), 0);
        ...
        const yesterdayCustomerParams = [yesterdayStart, yesterdayEnd];
        if (userRole !== 'super_admin' && userRole !== 'admin') {
        ...
        const [yesterdayCustomersResult] = await database_1.AppDataSource.query(`SELECT COUNT(*) as count FROM customers WHERE created_at >= ? AND created_at <= ?${customerCondition}`, yesterdayCustomerParams);
...
router.get('/charts', async (req, res) => {
    try {
        ...
        const categories = [];
        const orderRevenueData = []; // 下单业绩（金额）
        const deliveredRevenueData = []; // 签收业绩（金额）
        if (period === 'month') {
            // 本月每天的数据
            ...
            for (let i = 1; i <= daysInMonth; i++) {
                ...
                categories.push(`${i}日`);
                const dayOrdersData = await database_1.AppDataSource.query(`SELECT total_amount as totalAmount, status, mark_type as markType
           FROM orders o
           WHERE o.created_at >= ? AND o.created_at <= ?${userCondition}`, [dayStart, dayEnd, ...baseParams]);
                // 下单业绩
                const validOrders = dayOrdersData.filter((o) => isValidForOrderPerformance(o));
                orderRevenueData.push(validOrders.reduce((sum, o) => sum + (Number(o.totalAmount) || 0), 0));
                // 签收业绩
                const deliveredOrders = dayOrdersData.filter((o) => isValidForDeliveryPerformance(o));
           ...
           WHERE o.created_at >= ? AND o.created_at <= ?${userCondition}`, [dayStart, dayEnd, ...baseParams]);
            ...
            for (let i = 0; i < 24; i++) {
                ...
                const hourOrdersData = await database_1.AppDataSource.query(`SELECT total_amount as totalAmount, status, mark_type as markType
           FROM orders o
           WHERE o.created_at >= ? AND o.created_at <= ?${userCondition}`, [hourStart, hourEnd, ...baseParams]);
                // 下单业绩
                const validOrders = hourOrdersData.filter((o) => isValidForOrderPerformance(o));
                orderRevenueData.push(validOrders.reduce((sum, o) => sum + (Number(o.totalAmount) || 0), 0));
                // 签收业绩
                const deliveredOrders = hourOrdersData.filter((o) => isValidForDeliveryPerformance(o));
                deliveredRevenueData.push(deliveredOrders.reduce((sum, o) => sum + (Number(o.totalAmount) || 0), 0));
            }
        }
        ...
        const monthlyOrdersData = await database_1.AppDataSource.query(`SELECT status, total_amount as totalAmount
       FROM orders o
       WHERE o.created_at >= ? AND o.created_at <= ?${userCondition}`, [monthStart, monthEnd, ...baseParams]);
        const statusMap = {
