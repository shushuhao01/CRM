-- ============================================================
-- 生产环境数据库合并迁移 - 2026-03-09 (v2 - phpMyAdmin兼容版)
-- 
-- ★★★ 使用方法 ★★★
-- 在 phpMyAdmin 中: SQL标签 → 粘贴全部内容 → 点击执行
-- 定界符保持默认 ; 即可，无需修改任何设置
-- 
-- 包含内容:
...
-- ============================================
-- 10. 列变更: licenses 表新增 package_id
-- 先检查列是否存在，不存在才添加
-- ============================================
SET @col_count = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'licenses' AND COLUMN_NAME = 'package_id');
SET @sql_stmt = IF(@col_count = 0, 'ALTER TABLE `licenses` ADD COLUMN `package_id` INT NULL COMMENT ''关联套餐ID'' AFTER `features`', 'SELECT ''licenses.package_id already exists''');
PREPARE _stmt FROM @sql_stmt;
EXECUTE _stmt;
DEALLOCATE PREPARE _stmt;

-- ============================================
-- 11. 列变更: licenses 表新增 package_name
-- ============================================
SET @col_count = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'licenses' AND COLUMN_NAME = 'package_name');
SET @sql_stmt = IF(@col_count = 0, 'ALTER TABLE `licenses` ADD COLUMN `package_name` VARCHAR(100) NULL COMMENT ''套餐名称'' AFTER `package_id`', 'SELECT ''licenses.package_name already exists''');
PREPARE _stmt FROM @sql_stmt;
EXECUTE _stmt;
DEALLOCATE PREPARE _stmt;

-- ============================================
-- 12. 列变更: tenant_packages 表新增 modules
-- ============================================
SET @col_count = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'tenant_packages' AND COLUMN_NAME = 'modules');
SET @sql_stmt = IF(@col_count = 0, 'ALTER TABLE `tenant_packages` ADD COLUMN `modules` JSON NULL COMMENT ''授权模块ID列表（JSON数组）'' AFTER `features`', 'SELECT ''tenant_packages.modules already exists''');
PREPARE _stmt FROM @sql_stmt;
EXECUTE _stmt;
DEALLOCATE PREPARE _stmt;

-- ============================================
-- 13. 列变更: tenants 表新增 modules
-- ============================================
SET @col_count = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'tenants' AND COLUMN_NAME = 'modules');
SET @sql_stmt = IF(@col_count = 0, 'ALTER TABLE `tenants` ADD COLUMN `modules` JSON NULL COMMENT ''授权模块ID列表（JSON数组）'' AFTER `features`', 'SELECT ''tenants.modules already exists''');
PREPARE _stmt FROM @sql_stmt;
EXECUTE _stmt;
DEALLOCATE PREPARE _stmt;
...
-- ============================================
-- 15. 默认数据: module_status 模块启停状态（11个）
-- ============================================
...
-- 列变更通过 SET + PREPARE 实现安全检查，无需存储过程
-- 可安全重复执行，不会影响已有数据
-- ============================================================
