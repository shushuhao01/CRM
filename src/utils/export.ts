...
import { loadAddressData, getAddressPartLabels } from './addressData'
import { formatDateTime as formatBeijingDateTime, formatDate as formatBeijingDate } from './date'

/** 导出用北京时间（空值导出为空字符串，避免 Excel 出现 "-"） */
const exportDateTime = (v: unknown): string => {
  if (v === undefined || v === null || v === '') return ''
  const s = formatBeijingDateTime(v as string | Date)
  return s === '-' ? '' : s
}
const exportDate = (v: unknown): string => {
  if (v === undefined || v === null || v === '') return ''
  const s = formatBeijingDate(v as string | Date)
  return s === '-' ? '' : s
}

// 🔥 备注/疾病史等"多条记录JSON"字段格式化：解析JSON数组，按时间倒序（最新在前）拼接中文内容
// 数据库中备注/疾病史以 [{content, createTime, operator}] 的JSON字符串存储，
// 直接导出会显示原始JSON代码，这里转换为可读的中文内容拼接
const formatRecordList = (raw: unknown): string => {
  ...
  try {
    ...
    const items = list
      .map((item: any) => {
        if (typeof item === 'string') return { content: item, time: 0 }
        const content = item?.content || item?.description || ''
        const timeStr = item?.createTime || item?.createdAt || ''
        return { content: String(content), time: timeStr ? new Date(timeStr).getTime() : 0, timeStr }
      })
    ...
    return items
      .map(item => {
        const t = (item as any).timeStr ? String((item as any).timeStr).replace('T', ' ').substring(0, 16) : ''
        return t ? `${item.content}（${t}）` : item.content
      })
      .join('；')
  } catch {
    return str
  ...
  const data = orders.map(order => {
    ...
    if (isAdmin) {
      return [
        ...
        exportDateTime(order.createTime),
        order.products,
        order.totalQuantity,
        order.totalAmount,
        order.depositAmount,
        ...
        // 🔥 病史：多条记录JSON转为中文内容拼接（最新在前）
        formatRecordList(order.medicalHistory),
        order.serviceWechat || '',
...
// 导出客户到Excel
export const exportCustomersToExcel = async (customers: ExportCustomer[], filename: string = '客户列表', hasExportPermission: boolean = false, customFieldsConfig?: any[]) => {
  if (!customers || customers.length === 0) {
    throw new Error('没有可导出的数据')
  }

  // 🔥 加载省市区街道数据，用于把英文代码转换为中文名称
  try {
    await loadAddressData()
  } catch {
    // 地址数据加载失败时降级为原始值导出，不阻断导出流程
  }
  ...
  const data = customers.map(customer => {
    if (hasExportPermission) {
      // 🔥 省市区街道：数据库存的是英文代码（如 guangdong），转换为中文名称
      const addressLabels = getAddressPartLabels(customer.province, customer.city, customer.district, customer.street)
      const row = [
        customer.code || '',
        customer.name || '',
        customer.phone || '',
        ...
        exportDateTime(customer.createTime),
        customer.createdBy || '',
        customer.wechatId || '',
        ...
        customer.position || '',
        customer.source || '',
        exportDateTime(customer.fanAcquisitionTime),
        // 🔥 疾病史/备注：多条记录JSON转为中文内容拼接（最新在前）
        formatRecordList(customer.medicalHistory),
        customer.improvementGoals ? customer.improvementGoals.join(', ') : '',
        customer.otherGoals || '',
        exportDate(customer.lastServiceDate),
        customer.tags ? customer.tags.join(', ') : '',
        formatRecordList(customer.remarks),
      ...
      if (activeCustomFields.length > 0) {
        activeCustomFields.forEach((f: any) => {
          ...
          if (val === undefined || val === null || val === '') {
            ...
            row.push(f.fieldType === 'date' ? exportDate(val) : exportDateTime(val))
          } else {
            row.push(String(val))
          }
        })
