...
/** 尝试通过SDK发送指定payload（和SidebarCollect保持一致） */
...
async function preGenerateFormCard() {
  try {
    ...
    let mpPage = `pages/index/index.html?tenantId=${tenantId}&memberId=${memberId}&ts=${ts}&sign=${sign}&externalUserId=${extUserId}`
    try {
      ...
      if (tk) mpPage = `pages/index/index.html?t=${tk}`
    } catch { /* 降级使用完整路径 */ }
