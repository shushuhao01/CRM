          ...
          <el-card class="summary-card">
            <div class="summary-content">
              ...
              <div class="summary-info">
                ...
                <div class="summary-label">待审核订??/div>
              </div>
            </div>
          </el-card>
        </el-col>
          ...
          <el-card class="summary-card">
            <div class="summary-content">
              ...
              <div class="summary-info">
                ...
                <div class="summary-label">待审核金??/div>
              </div>
            </div>
          </el-card>
        </el-col>
    ...
    <el-card class="table-card-container">
      ...
      <template #header-actions>
        ...
        <div class="header-actions-right">
          ...
          <!-- 总记录数 -->
          <span class="total-info">??{{ pagination.total }} 条记??/span>
        </div>
      </template>
...
interface AuditOrder {
  ...
  createdBy?: string      // 创建??  createdById?: string    // 创建人ID
  totalAmount: number
...
// 图片查看器相??const imageViewerVisible = ref(false)
const currentImageList = ref<string[]>([])
const currentImageIndex = ref(0)

// 审核轨迹折叠状??const auditHistoryCollapsed = ref(true)

// 标签状态管??const activeTab = ref('pending')
...
// 红点状态管理（记录待审核标签的红点是否已被点击??const badgeClicked = reactive({
  pending: false
})

// 所有订单数??const allOrders = ref<AuditOrder[]>([])
...
// 销售人员列??- 从userStore获取真实用户数据
// ?? 【修复】过滤掉禁用用户，只显示启用的用??const salesUserList = computed(() => {
  return userStore.users
    ...
    .map(u => ({
      ...
      department: u.department || '未分??
    }))
})

// 计算属??const isIndeterminate = computed(() => {
...
const auditDialogTitle = computed(() => {
  if (isBatchAudit.value) {
    return `批量审核订单 (${selectedOrders.value?.length || 0}??`
  }
  return '订单审核'
})

// 汇总数??const summaryData = reactive({
  pendingCount: 0,
  pendingAmount: 0,
  todayCount: 0,
  urgentCount: 0
...
// 快捷筛??const activeQuickFilter = ref('today')
const quickFilters = [
...
// 表格列配??const tableColumns = computed(() => [
  {
    prop: 'orderNo',
    label: '订单??,
    width: 160,
...
// 当前操作的订??const currentOrder = ref<AuditOrder>({} as AuditOrder)
...
// 快速审核表??const quickAuditForm = reactive<QuickAuditForm>({
  result: null, // 使用null确保初始状态下没有选中任何选项
  rejectionReason: '',
  remark: ''
})
...
const auditRules = computed<FormRules>(() => ({
  ...
  remark: [] // 审核通过时非必填，拒绝时选择拒绝原因后也非必??}))

// 备注表单验证规则
const remarkRules: FormRules = {
  content: [
    { required: true, message: '请输入备注内??, trigger: 'blur' },
    { min: 2, message: '备注内容至少2个字??, trigger: 'blur' }
  ]
}

// 快速审核表单验证规??const quickAuditRules = computed<FormRules>(() => ({
  result: [
    { required: true, message: '请选择审核结果', trigger: 'change' }
  ],
...
const goToCustomerDetail = (row: AuditOrder) => {
  ...
  if (row.customerId) {
    ...
    if (order && order.customerId) {
      ...
      ElMessage.warning('客户ID不存??)
    }
  }
}

/**
 * 查看订单详情（打开审核弹窗?? 不设置默认选中状?? */
...
const handleAudit = (row: AuditOrder, result: 'approved' | 'rejected') => {
  currentOrder.value = row
  // 根据点击的按钮设置默认选中状??  auditForm.result = result
  auditForm.remark = ''
...
const handleReAudit = (row: AuditOrder, result: 'approved' | 'rejected') => {
  ...
  ElMessageBox.confirm(
    ...
    try {
      ...
      // 从当前列表移??      const currentList = orderList.value
  ...
  await auditFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        // 更新订单状??        ordersToUpdate.forEach(order => {
          ...
          // 更新订单存储中的订单状态，确保数据持久??          orderStore.auditOrder(order.id, isApproved, isApproved ? (auditForm.remark || '') : rejectionReason)
        ...
        ordersToUpdate.forEach(order => {
          ...
          // 获取下单员ID（优先使用salesPersonId，其次使用createdBy??          const orderCreatorId = order.salesPersonId || order.createdBy || order.createdById

          // 发送审核结果通知给下单员
          notificationStore.sendMessage(
            messageType,
            `订单 ${order.orderNo} (客户: ${order.customerName}, 金额: ￥${order.totalAmount?.toLocaleString()}) ??{actionText}`,
            {
              relatedId: order.id,
              relatedType: 'order',
              actionUrl: `/order/detail/${order.id}`,
              targetUserId: orderCreatorId,  // 发送给下单??              createdBy: userStore.user?.id  // 审核??            }
          )

          // 【批??01修复】发送待发货通知（审核通过时）
          if (auditForm.result === 'approved') {
            notificationStore.sendMessage(
              ...
              `订单 ${order.orderNo} 审核通过，已流转到物流发货列表，等待发货`,
              {
                ...
                targetUserId: orderCreatorId  // 发送给下单??              }
            )
          } else {
            // 发送退回通知给销售员（审核拒绝时??            const rejectionReason = auditForm.rejectionReasonId
              ? (rejectionReasonStore.getReasonById(auditForm.rejectionReasonId)?.name || auditForm.remark || '')
              : (auditForm.remark || '')
  ...
  await quickAuditFormRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        // 更新订单状??        order.auditStatus = result
        order.auditTime = new Date().toLocaleString()
        ...
        notificationStore.sendMessage(
          ...
          `订单 ${order.orderNo} (客户: ${order.customerName}, 金额: ￥${order.totalAmount?.toLocaleString()}) ??{actionText}`,
          {
            ...
            relatedType: 'order',
            actionUrl: `/order/detail/${order.id}`,
            targetUserId: orderCreatorId,  // 发送给下单??            createdBy: userStore.userInfo?.id  // 审核??          }
        )

        // 如果审核通过，更新订单状态并流转到物流和发货列表
        if (result === 'approved') {
          // 更新订单存储中的订单状??- 第二个参数必须是boolean类型
...
const handleQuickFilter = (filterValue: string) => {
  ...
  // 根据筛选值设置日期范??  const today = new Date()
...
const handleAuditResultChange = (result: string) => {
  // 当审核结果变化时，清空拒绝原??  if (result !== 'rejected') {
    auditForm.rejectionReasonId = ''
  }
...
const calculateSummaryData = () => {
  ...
  // 更新汇总数??  Object.assign(summaryData, {
    pendingCount,
    pendingAmount,
    todayCount,
    urgentCount
...
const loadSummaryData = async () => {
  try {
    ...
    console.log('汇总数据加载成??, summaryData)
  } catch (error) {
    console.error('加载汇总数据失??', error)
    // 如果API失败，使用本地计??    calculateSummaryData()
  }
}
...
const applyDataScopeControl = (orderList: any[]) => {
  ...
  // 超级管理员和管理员可以查看所有订??  if (currentUser.role === 'super_admin' || currentUser.role === 'admin') {
    console.log('[数据权限] 超管/管理员角色，可查看全部订??', orderList.length)
    return orderList
  }

  // 部门负责人可以查看本部门所有订??  if (currentUser.role === 'department_manager') {
    const filtered = orderList.filter(order => {
      // 使用 salesPersonId 来查找订单创建??      const orderCreator = userStore.getUserById(order.salesPersonId || order.createdBy)
  ...
  // 销售员只能查看自己创建的订单（使用 salesPersonId 进行匹配??  if (currentUser.role === 'sales_staff' || currentUser.role === 'employee') {
    const filtered = orderList.filter(order => {
    ...
    console.log('[数据权限] 销售员，可查看自己的订??', filtered.length)
    return filtered
  }

  // 客服可以查看所有待审核的订单（用于审核??  if (currentUser.role === 'customer_service') {
    console.log('[数据权限] 客服角色，可查看全部待审核订??', orderList.length)
    return orderList  // 客服需要看到所有订单才能进行审??  }

  // 其他角色默认只能查看自己创建的订单（使用 salesPersonId??  const filtered = orderList.filter(order => order.salesPersonId === currentUser.id || order.createdBy === currentUser.name)
  console.log('[数据权限] 其他角色，可查看自己的订??', filtered.length)
  return filtered
}
...
const loadOrderList = async () => {
  ...
  try {
    ...
    // ?? 调试：打印所有订单的状??    console.log(`[订单审核] 从Store获取??${allOrders.length} 个订单`)
    if (allOrders.length > 0) {
      console.log('[订单审核] 订单状态分??', allOrders.reduce((acc, order) => {
    ...
    const ordersForAudit = allOrders.filter(order => {
      // 排除预留??      if (order.markType === 'reserved') {
        return false
      }

      // 排除退??- 退单应该保留在成员系统，不流转到审??      if (order.markType === 'return') {
        return false
      }

      // ?? 修复：放宽状态过滤条件，支持更多待审核状??      // 待审核状态包括：pending_audit, confirmed, pending（兼容不同后端实现）
    ...
    console.log(`[订单审核] 筛选结果：??${ordersForAudit.length} 个待审核订单（已按时间倒序）`)
    ...
    const convertedPendingOrders = ordersForAudit.map(order => ({
      ...
      // ?? 使用真实姓名而不是用户名ID - 从用户列表查找真实姓??      salesPerson: (() => {
        // 优先使用createdByName
        if (order.createdByName) return order.createdByName
        // 从用户列表查??        const user = userStore.users.find(u => u.id === order.createdBy || u.username === order.createdBy)
      ...
      // ?? 等待时间从订单流转到待审核时开始计时（使用auditTransferTime，如果没有则使用createTime），单位：分??      waitingMinutes: Math.floor((new Date().getTime() - new Date(order.auditTransferTime || order.createTime).getTime()) / (1000 * 60)),
      remark: order.remark || '',
      auditStatus: order.auditStatus,
    ...
    const mockPendingOrders = [
      {
        ...
        remark: '客户要求尽快发货',
        auditStatus: 'pending',
        deliveryAddress: '北京市朝阳区建国??8号SOHO现代城A??201??,
        paymentScreenshots: [
          { id: 1, url: '/api/uploads/payment1.jpg', name: '支付宝转账截??jpg' },
      ...
      },
      {
        ...
        remark: '',
        auditStatus: 'pending',
        deliveryAddress: '上海市浦东新区陆家嘴环路1000号恒生银行大??0??,
        paymentScreenshots: [
      ...
      },
      {
        ...
        remark: '大客户订单，优先处理',
        auditStatus: 'pending',
        deliveryAddress: '广州市天河区珠江新城花城大道85号高德置地广场A??801??,
        paymentScreenshots: [
          { id: 4, url: '/api/uploads/payment4.jpg', name: '银行转账截图.jpg' },
          { id: 5, url: '/api/uploads/payment5.jpg', name: '支付宝截??jpg' }
        ],
        auditHistory: [
          {
            id: 1,
    ...
    const mockApprovedOrders = [
      {
        ...
        deliveryAddress: '深圳市南山区科技园南区深南大??988??,
        paymentScreenshots: [
      ...
      },
      {
        ...
        auditor: '审核员B',
        auditRemark: 'VIP客户订单，优先处??,
        deliveryAddress: '杭州市西湖区文三??59号昌地火炬大??号楼17??,
        paymentScreenshots: [
    ...
    const mockRejectedOrders = [
      {
        ...
        deliveryAddress: '成都市高新区天府大道中段1388号美年广场C??,
        paymentScreenshots: [],
        auditHistory: [
          {
            id: 1,
    ...
    // 设置订单数据 - 只使用真实数据，不使用模拟数??    console.log(`[订单审核] 转换后的真实订单数量: ${convertedPendingOrders.length}`)
    ...
    console.log(`[订单审核] 最终数据统计：待审??${pendingOrders.value.length}, 已通过=${approvedOrders.value.length}, 已拒??${rejectedOrders.value.length}`)
    ...
    // 确保在错误情况下数组仍然是有效的空数??    pendingOrders.value = []
...
onMounted(async () => {
  ...
  try {
    ...
    if (response && response.data && response.data.list) {
      ...
      console.log(`[订单审核] ??获取??${auditOrders.length} 条待审核订单`)
      ...
      console.log(`[订单审核] 待审核订单数?? ${pendingOrders.value.length}`)

      // 更新标签计数
      updateTabCounts()
    } else {
      console.warn('[订单审核] ?? API返回数据格式异常或为??', response)
      pendingOrders.value = []
    }
  } catch (error) {
    console.error('[订单审核] 从API加载待审核订单失??', error)
    ElMessage.error('加载待审核订单失??)
    pendingOrders.value = []
  } finally {
    loading.value = false
  }

  // 加载汇总数??  loadSummaryData()

  // 监听订单事件总线 - 实现订单状态同??  eventBus.on(EventNames.ORDER_TRANSFERRED, handleOrderTransferredAudit)
