-- =============================================
-- 权限数据迁移脚本
-- 将前端硬编码的 PERMISSION_TREE 数据写入 permissions 表
-- 创建时间：2024-12-20
-- 适用数据库：crm_local / 生产环境
-- 兼容：phpMyAdmin / MySQL 命令行
-- =============================================

-- 安全检查：清空现有权限数据（当前 permissions 表为空）
...
-- =============================================
-- 4. 服务管理 (communication)
-- =============================================
INSERT INTO `permissions` (`id`, `tenant_id`, `name`, `code`, `description`, `module`, `type`, `path`, `icon`, `sort`, `status`, `parentId`) VALUES
(47, NULL, '服务管理', 'communication', '服务管理模块（通讯管理）', 'communication', 'module', '/service-management', 'Headset', 4, 'active', NULL),
...
-- =============================================
-- 11.1 企微管理 - 标签选项卡权限 (tab)
-- =============================================
INSERT INTO `permissions` (`id`, `tenant_id`, `name`, `code`, `description`, `module`, `type`, `path`, `icon`, `sort`, `status`, `parentId`) VALUES
-- 通讯录 标签页 (parentId=179)
...
-- 重置自增ID到最大值之后
ALTER TABLE `permissions` AUTO_INCREMENT = 400;

-- =============================================
-- 13. 修复角色权限代码（将旧格式权限码更正为与permissions表一致的编码）
-- =============================================
-- 修复 customer_service: afterSales.* → aftersale.*, dashboard.personal → dashboard.view
...
-- 修复 department_manager: finance.cod_application → order.cod_application
UPDATE `roles` SET `permissions` = JSON_ARRAY(
...
-- 修复 sales_staff: finance.cod_application → order.cod_application
UPDATE `roles` SET `permissions` = JSON_ARRAY(
  'dashboard', 'dashboard.view',
...
-- 预期结果: 303 条记录 (256 + 47 tab权限)
-- SELECT `type`, COUNT(*) AS cnt FROM `permissions` GROUP BY `type`;
-- 预期结果: module=12, menu=约57, action=约187, tab=47
