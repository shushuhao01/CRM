...
> | 版本 | 日期 | 摘要 | 重要程度 |
> |------|------|------|----------|
> | v1.0 | 2025 | 初版，5角色基础权限树 | - |
> | v1.1 | 2026-02-25 | 新增财务管理模块权限配置 | ⭐ 中 |
> | v1.2 | 2026-04-02 | 重大更新：第三章默认权限列表与代码完全同步（`.`分隔格式），补充通话管理独立模块、财务模块权限 | ⭐⭐⭐ 极高 |

> 文档版本：1.2
> 更新日期：2026-04-02
> 更新内容：默认权限列表与 defaultRolePermissions.ts 代码完全同步
> 适用系统：智能销售CRM
...
**已配置权限列表**（与 `defaultRolePermissions.ts` 完全一致）:
...
# 通话管理（禁止：短信管理）
communication                       - 通话管理（一级菜单）
communication.call                  - 通话管理
communication.call.view             - 查看通话记录
communication.call.make             - 发起通话
communication.call.record           - 录音管理
...
performance.analysis.view           - 查看分析
performance.share                   - 业绩分享
performance.share.view              - 查看分享
...
data                                - 资料管理（一级菜单）
data.search                         - 客户查询
data.search.basic                   - 基础查询
data.search.advanced                - 高级查询

# 财务管理（绩效数据和取消代收申请，本部门/个人数据）
finance                             - 财务管理（一级菜单）
finance.performance_data            - 绩效数据
finance.performance_data.view       - 查看绩效数据
finance.cod_application             - 取消代收申请
...
order                               - 订单管理（一级菜单）
order.audit                         - 订单审核
order.audit.view                    - 查看待审核
order.audit.approve                 - 通过审核
order.audit.reject                  - 拒绝审核
