...
CREATE TABLE `products` (
  ...
  `weight` DECIMAL(10,2) DEFAULT 0 COMMENT '商品重量(kg)',
  `dimensions` VARCHAR(100) NULL COMMENT '商品尺寸',
  `specifications` JSON COMMENT '规格参数',
  `sku_type` VARCHAR(10) DEFAULT 'none' COMMENT 'SKU类型: none-无SKU, single-单SKU, multi-多SKU',
  `min_price` DECIMAL(10,2) DEFAULT NULL COMMENT 'SKU最低价',
  `max_price` DECIMAL(10,2) DEFAULT NULL COMMENT 'SKU最高价',
  `total_stock` INT DEFAULT NULL COMMENT 'SKU总库存',
...
CREATE TABLE `after_sales_services` (
  ...
  `resolution_type` VARCHAR(50) NULL COMMENT '解决方案类型: return_refund退货退款, return_replenish退货补货, exchange更换产品, repair维修, other其他',
  `refund_amount` DECIMAL(10,2) DEFAULT 0 COMMENT '退款金额',
  `refund_type` VARCHAR(20) NULL COMMENT '退款类型: full全额退款, partial部分退款',
  `resolution_product` VARCHAR(200) NULL COMMENT '处理结果商品',
  `resolution_remark` TEXT NULL COMMENT '处理结果备注',
...
CREATE TABLE `order_items` (
  ...
  `sku_image` VARCHAR(500) DEFAULT NULL COMMENT 'SKU图片快照',
  `spec_values` JSON DEFAULT NULL COMMENT 'SKU规格值快照',
...
CREATE TABLE IF NOT EXISTS `wecom_suite_configs` (
  ...
  `zone_get_msg_body_ability_id` VARCHAR(100) DEFAULT NULL COMMENT '获取消息体专用能力ID（如invoke_get_msg_body，为空则使用zone_sync_msg_ability_id）',
  `is_enabled` TINYINT(1) DEFAULT 0 COMMENT '是否启用',
  ...
  `web_login_appid` VARCHAR(100) DEFAULT NULL COMMENT 'Web登录授权AppID(服务商后台登录授权页面的SuiteID)',
  `web_login_secret` VARCHAR(255) DEFAULT NULL COMMENT 'Web登录授权Secret(服务商后台登录授权页面的Secret)',
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'
...
CREATE TABLE IF NOT EXISTS `wecom_chat_audit_marks` (
  ...
  `from_user_id` VARCHAR(100) NULL COMMENT '消息发送者',
  ...
  `operator_id` VARCHAR(50) NULL COMMENT '标记操作人ID',
  `operator_name` VARCHAR(100) NULL COMMENT '标记操作人名称',
